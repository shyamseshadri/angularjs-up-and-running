...
angular.module('stockMarketApp')
  .directive('simpleStockRepeat', [function() {
    return {
      ...
      transclude: 'element',
      // A $transclude is passed in as the fifth argument to the link
      link: function($scope, $element, $attrs, ctrl, $transclude) {
        var myArray = $scope.$eval($attrs.simpleStockRepeat);

        var container = angular.element('<div class="container"></div>');
        for (var i = 0; i < myArray.length; i++) {
          // Create an element instance with a new child scope using the
          // clone linking function
        ...
        // With transclude: 'element', the element gets replaced with a
        // comment. Add our generated content after the comment
